...
func TestParseApplication(t *testing.T) {
	v, err := ParseApplication("pepecoin/1.2.3")

	require.NoError(t, err)
	require.NotNil(t, v)
	require.Equal(t, "pepecoin/1.2.3", v.String())
	require.Equal(t, 1, v.Major)
	require.Equal(t, 2, v.Minor)
	require.Equal(t, 3, v.Patch)
	...
	tests := []struct {
		...
		{
			version:     "pepecoin/z.0.0",
			expectedErr: strconv.ErrSyntax,
		},
		{
			version:     "pepecoin/0.z.0",
			expectedErr: strconv.ErrSyntax,
		},
		{
			version:     "pepecoin/0.0.z",
			expectedErr: strconv.ErrSyntax,
		},
		{
			version:     "pepecoin/0.0.0.0",
			expectedErr: strconv.ErrSyntax,
		},
	}
	for _, test := range tests {
