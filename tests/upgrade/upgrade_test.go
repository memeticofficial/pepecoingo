...
var (
	...
	networkRunnerPepecoinGoExecPathToUpgrade string
...
func init() {
	...
	flag.StringVar(
		...
		"",
		"pepecoingo executable path",
	)
	flag.StringVar(
		&networkRunnerPepecoinGoExecPathToUpgrade,
		"network-runner-pepecoingo-path-to-upgrade",
		"",
		"pepecoingo executable path (to upgrade to, only required for upgrade tests with local network-runner)",
	)
	flag.StringVar(
		&networkRunnerPepecoinGoLogLevel,
		"network-runner-pepecoingo-log-level",
		"INFO",
		"pepecoingo log-level",
	)
}
...
var _ = ginkgo.BeforeSuite(func() {
	_, err := os.Stat(networkRunnerPepecoinGoExecPath)
	gomega.Expect(err).Should(gomega.BeNil())

	_, err = os.Stat(networkRunnerPepecoinGoExecPathToUpgrade)
	gomega.Expect(err).Should(gomega.BeNil())
	...
	ctx, cancel = context.WithTimeout(context.Background(), 15*time.Second)
	resp, err := runnerCli.Start(ctx, networkRunnerPepecoinGoExecPath,
		runner_sdk.WithNumNodes(5),
		runner_sdk.WithGlobalNodeConfig(fmt.Sprintf(`{"log-level":"%s"}`, networkRunnerPepecoinGoLogLevel)),
	)
	cancel()
	gomega.Expect(err).Should(gomega.BeNil())
...
var _ = ginkgo.Describe("[Upgrade]", func() {
	ginkgo.It("can upgrade versions", func() {
		...
		for _, name := range sresp.ClusterInfo.NodeNames {
			tests.Outf("{{magenta}}restarting the node %q{{/}} with %q\n", name, networkRunnerPepecoinGoExecPathToUpgrade)
			ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
			resp, err := runnerCli.RestartNode(ctx, name, runner_sdk.WithExecPath(networkRunnerPepecoinGoExecPathToUpgrade))
			cancel()
			gomega.Expect(err).Should(gomega.BeNil())
			...
			cancel()
			gomega.Expect(err).Should(gomega.BeNil())
			tests.Outf("{{green}}successfully upgraded %q to %q{{/}} (current info: %+v)\n", name, networkRunnerPepecoinGoExecPathToUpgrade, resp.ClusterInfo.NodeInfos)
		}
	})
})
