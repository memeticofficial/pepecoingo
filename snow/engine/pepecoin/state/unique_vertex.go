...
var (
	...
	_ pepecoin.Vertex        = (*uniqueVertex)(nil)
)
...
func (vtx *uniqueVertex) Parents() ([]pepecoin.Vertex, error) {
	vtx.refresh()
	...
	if len(vtx.v.parents) != len(parentIDs) {
		vtx.v.parents = make([]pepecoin.Vertex, len(parentIDs))
