...
import (
	...
	"github.com/memeticofficial/pepecoingo/snow/consensus/snowstorm"
	"github.com/memeticofficial/pepecoingo/snow/engine/pepecoin/getter"
...
func newConfig(t *testing.T) (Config, ids.NodeID, *common.SenderTest, *vertex.TestManager, *vertex.TestVM) {
	...
	vtxBlocker, err := queue.NewWithMissing(prefixdb.New([]byte("vtx"), db), "vtx", ctx.PepecoinRegisterer)
	require.NoError(err)

	txBlocker, err := queue.New(prefixdb.New([]byte("tx"), db), "tx", ctx.PepecoinRegisterer)
	require.NoError(err)
...
func TestBootstrapperSingleFrontier(t *testing.T) {
	...
	vtx2 := &pepecoin.TestVertex{ // vtx2 is the stop vertex
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID2,
			StatusV: choices.Processing,
		},
	...
	manager.GetVtxF = func(_ context.Context, vtxID ids.ID) (pepecoin.Vertex, error) {
		switch vtxID {
		case vtxID0:
			return vtx0, nil
		case vtxID1:
	...
	manager.ParseVtxF = func(_ context.Context, vtxBytes []byte) (pepecoin.Vertex, error) {
		switch {
...
func TestBootstrapperByzantineResponses(t *testing.T) {
	...
	vtx1 := &pepecoin.TestVertex{ // vtx1 is the stop vertex
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID1,
			StatusV: choices.Processing,
		},
		ParentsV: []pepecoin.Vertex{vtx0},
		HeightV:  1,
		BytesV:   vtxBytes1,
	}
	// Should not receive transitive votes from [vtx1]
	vtx2 := &pepecoin.TestVertex{
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID2,
			StatusV: choices.Unknown,
		},
...
func TestBootstrapperTxDependencies(t *testing.T) {
	...
	vtx1 := &pepecoin.TestVertex{ // vtx1 is the stop vertex
		...
		ParentsV: []pepecoin.Vertex{vtx0}, // Depends on vtx0
		HeightV:  1,
...
func TestRestartBootstrapping(t *testing.T) {
	...
	vtx3 := &pepecoin.TestVertex{
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID3,
			StatusV: choices.Unknown,
		},
		ParentsV: []pepecoin.Vertex{vtx2},
		HeightV:  3,
		BytesV:   vtxBytes3,
	}
	vtx4 := &pepecoin.TestVertex{
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID4,
			StatusV: choices.Unknown,
		},
	...
	vtx5 := &pepecoin.TestVertex{ // vtx5 is the stop vertex
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID5,
			StatusV: choices.Unknown,
		},
		ParentsV: []pepecoin.Vertex{vtx3, vtx4},
		HeightV:  4,
		BytesV:   vtxBytes5,
	}
